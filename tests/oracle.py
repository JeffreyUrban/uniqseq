...
def find_duplicates_naive(lines: List[str], window_size: int) -> Tuple[List[str], int]:
    ...
    while i < len(lines):
        ...
        for start_pos in range(i):
            ...
            match_len = 0
            while (i + match_len < len(lines) and
                   start_pos + match_len < i and
                   lines[start_pos + match_len] == lines[i + match_len]):
                match_len += 1
...
class LineProcessingInfo:
    ...
    buffer_depth_at_output: int | None  # Max buffer depth when this line was output (None if skipped)
    ...
    while i < len(lines):
        ...
        if i + window_size > len(lines):
            ...
            for j in range(i, len(lines)):
                ...
                buffer_depth = j - i
                line_processing.append(LineProcessingInfo(
                    line_number=j,
        ...
        if best_match_len >= window_size:
            ...
            for offset, j in enumerate(range(i, i + best_match_len)):
                line_processing.append(LineProcessingInfo(
                    line_number=j,
                    line_content=lines[j],
                    was_output=False,

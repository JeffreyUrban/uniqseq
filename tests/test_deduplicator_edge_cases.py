"""Additional unit tests to target specific uncovered edge cases in deduplicator."""
...
def test_sequence_confirmed_multiple_times():
    ...
    This targets line 469 in deduplicator.py - the else branch in _record_sequence
    ...
    dedup.flush(output)  # Records the sequence first time

    # Second chunk: A, B, C appears again
    for line in ["A", "B", "C", "Y"]:
        dedup.process_line(line, output)
    dedup.flush(output)  # Should increment repeat_count (line 469)

    # Third chunk: A, B, C appears again
    for line in ["A", "B", "C", "Z"]:
        dedup.process_line(line, output)
    dedup.flush(output)  # Should increment repeat_count again
...
def test_binary_preloaded_short_sequence():
    ...
    This targets lines 272-273, 282 in deduplicator.py - binary mode assertions
...
def test_preloaded_sequence_saved_on_match():
    ...
    This targets lines 540-543 in deduplicator.py - saving preloaded sequences
...
def test_history_eviction_during_matching():
    ...
    This targets line 572 in deduplicator.py - the continue statement when
...
def test_flush_with_existing_pattern():
    ...
    This targets lines 610-634 in deduplicator.py - the flush path in _check_candidates
    ...
    for line in ["A", "B", "C", "X"]:
        dedup.process_line(line, output)
    dedup.flush(output)  # Records ABC as a unique sequence

    # Second chunk: Have "A", "B", "C" in buffer at flush
    # This should match the existing sequence
    for line in ["Y", "A", "B", "C"]:
        dedup.process_line(line, output)
    dedup.flush(output)  # Should find ABC in buffer, match to existing, and skip it
...
def test_flush_with_preloaded_existing_pattern():
    ...
    This targets lines 618-627 in deduplicator.py - saving preloaded sequences
...
def test_lru_eviction_actually_happens():
    ...
    This targets line 672 in deduplicator.py - the popitem(last=False) call
    ...
    dedup = StreamingDeduplicator(window_size=3, max_history=100, max_unique_sequences=50)
    ...
    for i in range(0, len(lines), chunk_size):
        ...
        for line in chunk:
            dedup.process_line(line, output)
        dedup.flush(output)  # This triggers _record_sequence which checks max_unique_sequences

    # Check that eviction occurred - total sequences should be <= 50
    total_seqs = sum(len(seqs) for seqs in dedup.unique_sequences.values())
...
def test_match_index_overflow_edge_case():
    ...
    This targets lines 498-499 in deduplicator.py - the edge case handling
